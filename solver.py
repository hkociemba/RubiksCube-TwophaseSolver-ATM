        ...
        if togo_phase2 == 0 and slice_sorted == 0:
            ...
            if len(self.solutions) == 0 or (len(self.solutions[-1]) > len(man)):
                ...
                if self.inv == 1:  # we solved the inverse cube
                    ...
                    man[:] = [sy.inv_move[m] for m in man]  # Inverse moves
            ...
            for m in (
                    Move.U1D0, Move.U2D0, Move.U3D0, Move.U0D1, Move.U0D2, Move.U0D3, Move.R2L0, Move.R0L2,
                    Move.F2B0, Move.F0B2, Move.R2L2, Move.F2B2, Move.U1D1, Move.U1D2, Move.U1D3, Move.U2D1,
                    Move.U2D2, Move.U2D3, Move.U3D1, Move.U3D2, Move.U3D3):

                if len(self.sofar_phase2) > 0:
                    if m // 15 == self.sofar_phase2[-1] // 15:  # successive moves on same axis
                        continue
                else:
                    if m // 15 == self.sofar_phase1[-1] // 15:  # successive moves on same axis
                        continue

                corners_new = mv.corners_move[45 * corners + m]
                ud_edges_new = mv.ud_edges_move[45 * ud_edges + m]
                slice_sorted_new = mv.slice_sorted_move[45 * slice_sorted + m]
    ...
    def search(self, flip, twist, slice_sorted, dist, togo_phase1):
        ...
        if togo_phase1 == 0:  # phase 1 solved
            ...
            for m in self.sofar_phase1:  # get current corner configuration
                corners = mv.corners_move[45 * corners + m]

            # new solution must be shorter and we do not use phase 2 maneuvers with length > 10 - 1 = 9
            ...
            for m in self.sofar_phase1:
                u_edges = mv.u_edges_move[45 * u_edges + m]
                d_edges = mv.d_edges_move[45 * d_edges + m]
            ...
            for m in Move:
                # dist = 0 means that we are already are in the subgroup H. If there are less than 3 moves left
                ...
                if dist == 0 and togo_phase1 < 3 and m in (
                        Move.U1D0, Move.U2D0, Move.U3D0, Move.U0D1, Move.U0D2, Move.U0D3, Move.R2L0, Move.R0L2,
                        Move.F2B0, Move.F0B2, Move.R2L2, Move.F2B2, Move.U1D1, Move.U1D2, Move.U1D3, Move.U2D1,
                        Move.U2D2, Move.U2D3, Move.U3D1, Move.U3D2, Move.U3D3):
                    continue
                ...
                flip_new = mv.flip_move[45 * flip + m]  # N_MOVE = 45
                twist_new = mv.twist_move[45 * twist + m]
                slice_sorted_new = mv.slice_sorted_move[45 * slice_sorted + m]
