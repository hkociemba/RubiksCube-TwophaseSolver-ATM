...
# ########################################## Fill the inv_move array ####################################################

# Indices for the inverse moves
inv_move = ar.array('B', [0] * N_MOVE)
for j in range(N_MOVE):
    for i in range(N_SYM):
        cc = cb.CubieCube(cb.moveCube[j].cp, cb.moveCube[j].co, cb.moveCube[j].ep, cb.moveCube[j].eo)
        cc.corner_multiply(cb.moveCube[i])
        if cc.cp[Co.URF] == Co.URF and cc.cp[Co.UFL] == Co.UFL and cc.cp[Co.ULB] == Co.ULB and \
                cc.cp[Co.UBR] == Co.UBR and cc.cp[Co.DFR] == Co.DFR and cc.cp[Co.DLF] == Co.DLF and \
                cc.cp[Co.DBL] == Co.DBL and cc.cp[Co.DRB] == Co.DRB:
            inv_move[j] = i
            break
########################################################################################################################
