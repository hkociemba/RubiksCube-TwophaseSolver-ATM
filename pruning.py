...
def create_phase1_prun_table():
    ...
    if not path.isfile(fname):
        ...
        while done != total:
            depth3 = depth % 3
            if depth == 7:
                # backwards search is faster for depth >= 7
            ...
            for fs_classidx in range(defs.N_FLIPSLICE_CLASS):
                ...
                while twist < defs.N_TWIST:
                    ...
                    if match:
                        ...
                        for m in enums.Move:
                            twist1 = mv.twist_move[45 * twist + m]  # defs.N_MOVE = 45
                            flip1 = mv.flip_move[45 * flip + m]
                            slice1 = mv.slice_sorted_move[1080 * slice_ + m] // 24  # defs.N_PERM_4 = 24, 45*24 = 1080
...
def create_phase2_prun_table():
    ...
    if not path.isfile(fname):
        ...
        depth = 0
        print('depth:', depth, 'done: ' + str(done) + '/' + str(total))
        while depth < 9:  # we fill the table only do depth 8 + 1
            depth3 = depth % 3
            idx = 0
            mult = 2
            if depth > 9:
            ...
            for c_classidx in range(defs.N_CORNERS_CLASS):
                ...
                while ud_edge < defs.N_UD_EDGES:
                    ...
                    if get_corners_ud_edges_depth3(idx) == depth3:
                        ...
                        for m in (
                                enums.Move.U1D0, enums.Move.U2D0, enums.Move.U3D0, enums.Move.U0D1, enums.Move.U0D2,
                                enums.Move.U0D3, enums.Move.R2L0, enums.Move.R0L2, enums.Move.F2B0, enums.Move.F0B2,
                                enums.Move.R2L2, enums.Move.F2B2, enums.Move.U1D1, enums.Move.U1D2, enums.Move.U1D3,
                                enums.Move.U2D1, enums.Move.U2D2, enums.Move.U2D3, enums.Move.U3D1, enums.Move.U3D2,
                                enums.Move.U3D3):
                            ud_edge1 = mv.ud_edges_move[45 * ud_edge + m]
                            corner1 = mv.corners_move[45 * corner + m]
        ...
        print('remaining unfilled entries have depth >=10')
        fh = open(fname, "wb")
...
def create_phase2_cornsliceprun_table():
    ...
    if not path.isfile(fname):
        ...
        while done != defs.N_CORNERS * defs.N_PERM_4:
            for corners in range(defs.N_CORNERS):
                for slice_ in range(defs.N_PERM_4):
                    if cornslice_depth[defs.N_PERM_4 * corners + slice_] == depth:
                        for m in enums.Move:
                            if m in (
                                    enums.Move.R0L1, enums.Move.R0L3, enums.Move.R1L1, enums.Move.R1L2, enums.Move.R1L3,
                                    enums.Move.R1L0, enums.Move.R2L1,
                                    enums.Move.R2L3, enums.Move.R3L1, enums.Move.R3L2, enums.Move.R3L3, enums.Move.R3L0,
                                    enums.Move.F0B1, enums.Move.F0B3,
                                    enums.Move.F1B1, enums.Move.F1B2, enums.Move.F1B3, enums.Move.F1B0, enums.Move.F2B1,
                                    enums.Move.F2B3, enums.Move.F3B1, enums.Move.F3B2, enums.Move.F3B3,
                                    enums.Move.F3B0):
                                continue
                            corners1 = mv.corners_move[45 * corners + m]
                            slice_1 = mv.slice_sorted_move[45 * slice_ + m]
            ...
            depth += 1
            # print()
            # print('depth:', depth, 'done: ' + str(done) + '/' + str(defs.N_CORNERS * defs.N_PERM_4))
        print()
        fh = open(fname, "wb")
