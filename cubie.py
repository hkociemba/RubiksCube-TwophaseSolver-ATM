...
from enums import Axis, Color, Corner as Co, Edge as Ed
import face
...
moveCube = [CubieCube()] * 45
for j in Axis:
    cc = CubieCube()
    for k1 in range(4):
        cc.multiply(basicMoveCube[j])
        for k2 in range(4):
            cc.multiply(basicMoveCube[j+3])
            if not (k1 == 3 and k2 == 3):
                moveCube[15 * j + 4 * k1 + k2] = CubieCube(cc.cp, cc.co, cc.ep, cc.eo)
########################################################################################################################
